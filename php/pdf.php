...
session_start();
	...
	if( $zeile_wst['werkstoff'] == 4 ) {$werkstoff = "Gu&szlig;eisen";}
	...
	$maschnr = $zeile_wst['maschnr'];
	
	$image_beschr_1 = $zeile_app['beschr1'];
	$image_beschr_2 = $zeile_app['beschr2'];
	
	$kunde = $zeile_wst['kunde'];
	$wkunden = $zeile_app['wkunden'];
	...
	$array_besonder = explode(";", $zeile_app['besonderheiten']);
	...
	//Qualitätsvorgaben
	$strich_vorg = chr(149) ." ";
	$array_vorg = explode(";", $zeile_app['qualitaetsvorgaben']);
	...
	//Feritgungsziele
	$strich_fert = chr(149) ." ";
	$array_fert = explode(";", $zeile_app['fertigungsziele']);
	...
	$array_nutzen = explode(";", $zeile_app['kundennutzen']);
	...
	$pdf->SetFont('Arial', 'B', 10);
	$pdf->Cell(1,8, '');
	$pdf->Cell(30,8, 'Kunde', 'LBT', 0, 'L', 1);
	$pdf->SetFont('Arial', '', 10);
	$pdf->Cell(55,8, $kunde, 'BTR', 0, 'L', 1);
	...
	//weitere Kunde
	$pdf->SetFont('Arial', 'B', 10);
	$pdf->Cell(30,8, 'weitere', 'BTL', 0, 'L', 1);
	$pdf->SetFont('Arial', '', 10);
	$pdf->Cell(55,8, $wkunden, 'BTR', 1, 'L', 1);
